...
export function buildBillMessage(bill: Bill, tenant: Tenant, room: Room): string {
  return [
    ...
    ``,
    `This Month's Bill: ${formatCurrency(bill.total_amount)}`,
    ...(bill.paid_amount ? [`Paid: ${formatCurrency(bill.paid_amount)}`] : []),
    `Total Amount Due: ${formatCurrency(bill.outstanding_amount)}`,
    ``,
